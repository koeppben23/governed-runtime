...
- **Verification Command Planner (P12)**: Added advisory `verificationCandidates` surfaced via `flowguard_status`, derived deterministically from repository evidence with priority: package scripts > Java wrappers (`./mvnw`, `./gradlew`) > detected-tool fallbacks. Commands are planner output only (never auto-executed). Includes placeholder test-script filtering (ignore `no test specified` + `exit 1`) and Windows wrapper command forms (`mvnw.cmd`, `gradlew.bat`) when only batch wrappers are present.
