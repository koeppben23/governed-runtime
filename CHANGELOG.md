...
- **StrykerJS Mutation Testing (Phase 1 PoC)**: Introduced StrykerJS mutation testing for three security-critical files (`guards.ts`, `evaluate.ts`, `token-verifier.ts`) with vitest test runner, `perTest` coverage analysis, and TypeScript checker integration. Baseline score 85.71% (125 mutants killed by tests, 119 rejected by TypeScript). CI runs via `mutation` job (non-blocking, `continue-on-error`, 60min timeout) with `break: 85` threshold. Pre-flight `stryker-patch.js` script patches upstream vitest-runner `pool=threads` → `pool=forks` for `process.chdir()` compatibility, scoped exclusively to `npm run mutation`.
