...
- **P34a Foundation - Independent Self-Review Schema & Policy (Foundation, not Complete)**: Added `ReviewFindings`, `ReviewActorInfo`, and `Finding` schemas in `src/state/evidence.ts` with parallel storage in `PlanRecord.reviewFindings` (separate from `PlanRecord.history`). Added `selfReview: { subagentEnabled, fallbackToSelf }` to policy interface and all presets. Wired `selfReview` through `PolicySnapshot` for persistence. Added `latestReview` summary to `flowguard_status`. Added fallback semantics: `subagentEnabled=true` + subagent success → use findings; `subagentEnabled=true` + subagent failure + `fallbackToSelf=true` → degraded warning; `subagentEnabled=true` + subagent failure + `fallbackToSelf=false` → BLOCKED. **Status: Foundation only** — the actual subagent invocation is currently a stub (returns null). A complete independent review subagent path depends on OpenCode task/subagent orchestration being accessible from FlowGuard's controlled runtime; this is not clearly documented as an available API pattern in the official OpenCode docs. The stub enables policy-semantic testing and fallback-path validation. Full subagent integration requires P34a.2.
