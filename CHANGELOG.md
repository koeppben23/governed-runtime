...
- **Independent Review Governance**: Fully implemented agent-orchestrated independent review with plugin-level enforcement. Plan and implement tools accept structured `reviewFindings` from an independent review agent (OpenCode subagent orchestration). Installer deploys hidden `flowguard-reviewer` agent definition (`.opencode/agents/flowguard-reviewer.md`) and task permissions in `opencode.json`. Slash commands (`plan.md`, `implement.md`, `continue.md`) include dual-path orchestration: Path A (subagent via Task tool) when `subagentEnabled=true`, Path B (self-review) when disabled. Tool responses include policy-conditional `next` messages with `INDEPENDENT_REVIEW_REQUIRED:` prefix for subagent mode and `reviewMode` field for observability. Author and reviewer artifacts stored separately (append-only): `plan.history` / `plan.reviewFindings` and `implementation` / `implReviewFindings`. Policy-controlled via `selfReview.subagentEnabled` and `selfReview.fallbackToSelf` (disabled by default, backward-compatible). Two-layer fail-closed enforcement: (1) structural validation — review mode vs. policy, plan-version binding, iteration binding, mandatory findings for approval (`review-validation.ts`); (2) plugin-level enforcement — OpenCode `tool.execute.before/after` hooks observe the tool-call sequence and physically block verdict submission if the `flowguard-reviewer` subagent was not invoked (`SUBAGENT_REVIEW_NOT_INVOKED`) or if `reviewedBy.sessionId` does not match the actual subagent session (`SUBAGENT_SESSION_MISMATCH`). Enforcement logic in `review-enforcement.ts`, integrated via plugin hooks in `plugin.ts`. Status projections expose `latestReview` and `latestImplementationReview` summaries. Shared validation logic extracted to `review-validation.ts`. Added 123 tests (33 validation unit, 27 enforcement unit, 30 integration plan, 33 integration implement).
