...
- **Issue #245 (Phase 4: Multi-platform reviewer orchestration):** Add platform review orchestration projection for OpenCode, Claude Code, and Codex without introducing a second review authority. OpenCode remains on the existing `host_task_sync` / `host_subagent_task` paths. Claude/Codex emit `external_instruction_pending` guidance and install native `flowguard-reviewer` agent/subagent templates as transport/isolation artifacts only; review completion still requires validated, obligation-bound `ReviewFindings`. External `.flowguard/sessions/<session-id>/review-evidence/*.json` files are parsed and bound as transport evidence, never accepted by file existence alone. Installer supports `--platform opencode|claude-code|codex` and writes Claude `agents/flowguard-reviewer.md` or Codex `subagents/flowguard-reviewer.md` without touching OpenCode config for those platforms.
