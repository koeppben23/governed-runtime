...
- **Test workspace safety guard**: Added `assertTestConfigDir()` to integration test helpers. This guard is called automatically by `createTestWorkspace()` after redirecting `OPENCODE_CONFIG_DIR` and throws if the environment variable is unset or not pointing to a temporary directory, preventing accidental writes to the production workspace registry.
