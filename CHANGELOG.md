...
- **IdP-Verified Actor Identity (P35a)**: Added static-key JWT verification for enterprise IdP integration. Supports RS256 and ES256 signature verification via Node.js crypto. IdP configuration via `identityProvider` in policy (issuer, audience, claim mapping, signing keys as JWK or PEM). Token file path via `FLOWGUARD_ACTOR_TOKEN_PATH`. `identityProviderMode: 'optional' | 'required'` controls session creation behavior. `ActorVerificationMeta` provides IdP provenance (issuer, audience, keyId, algorithm, verifiedAt). Error codes: `IDP_TOKEN_MISSING`, `IDP_TOKEN_INVALID`, `IDP_TOKEN_HEADER_INVALID`, `IDP_KEY_NOT_FOUND`, `IDP_ALGORITHM_NOT_ALLOWED`, `IDP_SIGNATURE_INVALID`, `IDP_ISSUER_MISMATCH`, `IDP_AUDIENCE_MISMATCH`, `IDP_EXPIRED`, `IDP_NOT_YET_VALID`, `IDP_SUBJECT_MISSING`, `IDP_CLAIM_MAPPING_INVALID`. Module: `src/identity/`.
