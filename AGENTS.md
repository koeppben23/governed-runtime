...
These are contributor instructions for this repository, not FlowGuard runtime
policy.

## Scope

- Applies repository-wide to FlowGuard-owned source, tests, configuration,
  documentation, schemas, and repository-local tooling.
- Never propagate these repository rules into installed FlowGuard mandates,
  generated prompts, generated code, downstream repositories, or runtime
...
- Root `AGENTS.md` applies everywhere. Nested `AGENTS.md` files may add or
  specialize rules for their subtree but must not weaken repository-wide safety,
  evidence, legacy, authority, verification, or Git rules.
- If repository instructions conflict, identify the conflict and stop before
  violating either rule.

Normative terms: **MUST / MUST NOT** are requirements; **SHOULD / SHOULD NOT**
are defaults unless repository evidence justifies an exception. `ASSUMPTION`
means plausible but unverified. `NOT_VERIFIED` means a relevant check did not
run or did not establish the claim. `BLOCKED` means safe completion is not
possible with available evidence or capability.

## Required Workflow

For implementation, refactoring, and review:

1. Investigate first. Read relevant code, tests, schemas, config, and docs. Never
   speculate about code when repository evidence is available.
...
7. Report facts only: changed behavior, checks actually run, outcomes, and any
   remaining `ASSUMPTION`, `NOT_VERIFIED`, or `BLOCKED` item.
...
- Preserve deterministic and fail-closed semantics.
- Surface failures explicitly. No silent fallback, swallowed error, or
  default-allow behavior.
- Do not weaken tests, thresholds, validation, guards, or schemas to make a
  change pass.
- Implement general behavior; do not hard-code production logic only to satisfy
  specific test cases.
...
- Production boundary errors must be typed and carry a `code`; do not introduce
  ...
  tool boundaries.
- For blocked tool results use
  `{ kind: 'blocked', code: string, reason: string, recovery?: string }`.
- Production files above 750 LOC and test files above 2000 LOC are merge blockers.
...
obsolete FlowGuard API, schema, field, format, state shape, import path, or
runtime behavior working after a canonical replacement exists. This includes
compatibility shims, deprecated aliases, legacy fallbacks, old-format adapters,
dual-read/dual-write paths, read-time synthesis/normalization of obsolete shapes,
and migration layers that preserve obsolete behavior.
...
- If legacy compatibility code is encountered in a FlowGuard file or execution
  path being changed or reviewed, remove it in the same change. Update directly
  affected callers, schemas, tests, fixtures, and docs to the current authority.
- Do not leave discovered legacy code because it predates the task. If safe
  removal cannot be completed, the change is `BLOCKED` or `changes_requested`,
  not merge-ready.
- Keep cleanup bounded to the touched and directly dependent FlowGuard surface;
  widen scope only when required for correct removal.
- Tests and fixtures may contain obsolete inputs only to prove current boundaries
  reject them. Historical docs may describe removed behavior.

Examples: a forwarding alias kept only for obsolete callers is forbidden; a
branch that normalizes a retired field is forbidden; a test fixture asserting
that an obsolete schema version is rejected is allowed.
...
Do not treat generated, rendered, persisted, or compatibility projections as
SSOT when a canonical authority exists.
...
without reason. If a required check cannot run, report `NOT_VERIFIED` with the
