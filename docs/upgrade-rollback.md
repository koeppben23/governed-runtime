...
| Pre-current sessions (`schemaVersion: v2`/earlier, `audit-chain.v2`/earlier) | Current contract (`v3` / `audit-chain.v3`) | Incompatible by design. State rejected with `SESSION_STATE_INCOMPATIBLE` (audit records with `LEGACY_ASSURANCE_FORMAT_UNSUPPORTED`); no migration or re-seal path. Start a fresh session. |
