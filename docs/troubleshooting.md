...
| `OPENCODE_INSTRUCTION_SOURCE_UNSUPPORTED` | Detected OpenCode runtime does not resolve `instructions[]` as an instruction source        | Use a supported OpenCode runtime (Desktop and every CLI version resolve `instructions[]`); re-run `flowguard doctor` after switching                                 |
