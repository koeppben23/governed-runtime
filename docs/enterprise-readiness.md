...
- Actor assurance tiers: `best_effort` (env/git/unknown), `claim_validated` (verified claim file), `idp_verified` (future IdP/OIDC).
...
| Fake or weak human identity claims                    | Best-effort `actorInfo` for `env`/`git`/`unknown`; optional actor claims via `FLOWGUARD_ACTOR_CLAIMS_PATH` with `claim_validated` assurance; regulated approvals enforce `minimumActorAssuranceForApproval` threshold (`claim_validated` or `idp_verified`) | Verified claims are local trusted-claim files, not a full IAM/IdP integration; compromised local environments remain a residual risk |
