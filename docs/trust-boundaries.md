...
| Producer / outbox binding  | [`src/integration/audit-outbox.ts`](../src/integration/audit-outbox.ts)                                                                  |
...
| Required operational logs                 | `info` for archive lifecycle progress and `error` for archive creation/verification failures with fields such as `sessionId`, `packagePurpose`, `integrityCapability`, `verificationStatus`, `findingCode`, and `archivePath` where safe. Operational logs are diagnostic only.                                                                                                                                                                                                               |
| Known gaps / residual risk / NOT_VERIFIED | Keyless archive digesting is not a full mitigation for an attacker who can rewrite `audit.jsonl` and re-seal the manifest. With TSA/RFC 3161 evidence configured, verification enforces the RFC 3161 signer contract, rejects downgraded timestamp statuses (`TSA_EVIDENCE_DOWNGRADED` → `tsa_evidence_downgraded`), and derives finding severity exclusively from the trusted `resolveStrictMode(state)` resolution — the manifest policy mode is cross-checked, never a severity authority. |
...
| Writer / authority                        | Assurance helpers: [`src/integration/review/assurance.ts`](../src/integration/review/obligations/assurance.ts). Findings validation: [`src/integration/review/review-validation.ts`](../src/integration/review/validation/review-validation.ts). Review schemas: [`src/state/evidence.ts`](../src/state/evidence.ts). |
...
| Known gaps / residual risk / NOT_VERIFIED | A malicious or compromised host runtime is outside FlowGuard's full prevention boundary. Host capability probes are `NOT_VERIFIED` until runtime evidence exists. Presentation/admin console features are not built-in enforcement authority. The adversarial scenario ledger for this boundary is [`opencode-host-boundary-attack-matrix.md`](./opencode-host-boundary-attack-matrix.md). |
...
| **Policy downgrade in archive** | State-derived strictness and manifest policy-mode mismatch finding     | Re-seal risk remains without a configured and enforced TSA anchor |
