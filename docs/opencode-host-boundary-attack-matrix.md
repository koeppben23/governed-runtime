...
| HS-06 | Host retries the same tool call (same or new `callID`).                                 | No duplicate evidence or second authority effect; deterministic call identity.          | Reviewer retry is covered: `src/integration/review/orchestrator-retry-core.test.ts:99`; host-tool retry is untested.                           | Gap     | F-11    |
| HS-07 | Duplicate `callID` across before/after (retry, replay, identity confusion).             | Deterministic resolution or fail-closed deny; trace map cannot be poisoned.             | Trace fallback only: `src/integration/plugin-beforehooks.test.ts:200`, `src/integration/plugin-beforehooks.test.ts:1511`.                      | Gap     | F-11    |
...
| HS-09 | After hook arrives for a call the plugin never observed in before.                      | Ignore without corrupting trace state; no evidence fabrication.                         | After-hook context extraction: `src/integration/plugin-enforcement-tracking.test.ts:157`; no foreign-call test.                                | Gap     | F-11    |
...
| TS-03 | MCP-prefixed tool (`mcp__...`) at the hook boundary.                          | Same classification and phase gates as native names.              | Reviewer deny-rule smoke only: `src/cli/opencode-reviewer-capability.test.ts:82`; no before/after hook test.                                                | Gap     | F-09    |
...
| TS-07 | Reviewer child session attempts mutating tools or a nested `task`.            | Denied by installed reviewer agent permissions.                   | Real-host probe: `src/cli/opencode-reviewer-capability.test.ts:57`; orphaned from all vitest projects.                                                      | Partial | F-09    |
...
| CI-03 | Interleaved sessions A/B on one instance read mutable `getSessionId()`. | No cross-session authority; session id is call-scoped.                               | No test. Closure is mutated in `src/integration/plugin.ts:93` and `src/integration/plugin-beforehooks.ts:72`. | Gap     | F-07    |
...
| CI-05 | Duplicate `callID` concurrently.                                        | No double evidence or double authority; deterministic reject or idempotent handling. | No test.                                                                                                      | Gap     | F-11    |
...
| ID    | Scenario / vector                                             | Expected fail-closed behavior                                          | Coverage                                                                                          | Status  | Finding |
| ----- | ------------------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ------- | ------- |
| CP-01 | Compaction while a review obligation is pending.              | Mandatory pending-obligation context is injected.                      | `src/integration/plugin-compaction.test.ts:97`; no in-flight review test.                         | Partial | F-12    |
...
| CP-03 | Compaction during an active review attempt or challenge loop. | Attempt and obligation state survive; context never claims completion. | No end-to-end test.                                                                               | Gap     | F-12    |
...
| ID    | Scenario / vector                                                         | Expected fail-closed behavior                                               | Coverage                                                                                                                   | Status  | Finding |
| ----- | ------------------------------------------------------------------------- | --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ------- | ------- |
| PL-01 | Plugin boot with a broken SDK client (`session.create`/`prompt` missing). | Fail closed before hooks are exposed.                                       | Adapter unit only: `src/adapters/host-adapter.test.ts:153`; the composition root never calls `initialize()`.               | Gap     | F-01    |
| PL-02 | Capability mismatch at boot (for example agent registry unavailable).     | Fail closed or enter an explicit degraded mode before governance hooks run. | Adapter unit only: `src/adapters/host-adapter.test.ts:254`; the composition root never calls `validateCapabilities()`.     | Gap     | F-01    |
| PL-03 | Advertised capabilities versus what is actually probed.                   | Only verified capabilities may be claimed; the rest are marked unverified.  | Six advertised, one probed: `src/integration/opencode-host-adapter.ts:56`, `src/integration/opencode-host-adapter.ts:122`. | Gap     | F-04    |
| PL-04 | Dispose must shut down the adapter and logging.                           | Composed shutdown; no leaked resources.                                     | `hooks.dispose` is only `disposeLogging`: `src/integration/plugin.ts:140`; adapter `shutdown()` is never called.           | Gap     | F-01    |
...
| RC-02 | In-flight abort of a reviewer child.                             | Typed aborted outcome; unsupported abort fails closed.                        | Fake probe only: `src/integration/review/__tests__/parallel-host-probe.test.ts:175`, `src/integration/review/__tests__/parallel-host-probe.test.ts:201`.        | Partial | F-13    |
| RC-03 | Reviewer child exceeds its time budget.                          | Deterministic timeout classification; no hang; parent remains resumable.      | No functional timeout test; timeouts appear only in error-message assertions.                                                                                   | Gap     | F-13    |
| RC-04 | Parent session is deleted or crashes while the child is running. | No orphan authority; late callbacks from superseded sessions add no evidence. | Logical coverage: `src/integration/attempt-lifecycle-e2e.test.ts:553`, `src/integration/plugin-afterhooks-host-task.test.ts:522`; no process or orphan cleanup. | Partial | F-13    |
...
**Severity:** P1. **Status:** Confirmed bug. **Scenarios:** PL-01, PL-02, PL-04.
...
**Severity:** P1. **Status:** Confirmed bug. **Scenarios:** HD-01, HD-02.
...
**Severity:** P1. **Status:** Confirmed architectural gap. **Scenarios:** PL-03, PL-06.
...
**Severity:** P1/P2. **Status:** Confirmed policy gap. **Scenarios:** HD-06.
...
**Severity:** P2. **Status:** Confirmed bug. **Scenarios:** SL-04.
...
**Severity:** P2. **Status:** Latent risk. **Scenarios:** CI-03.
...
**Severity:** P1. **Status:** Coverage gap. **Scenarios:** HS-01.
...
**Severity:** P2. **Status:** Confirmed bug. **Scenarios:** TS-03, TS-07, HD-05.
...
**Severity:** P2. **Status:** Coverage gap. **Scenarios:** HD-07.
...
**Severity:** P2. **Status:** Coverage gap. **Scenarios:** HS-03, HS-06, HS-07, HS-09, CI-05.
...
**Severity:** P2. **Status:** Coverage gap. **Scenarios:** CP-01, CP-03.
...
**Severity:** P2. **Status:** Coverage gap. **Scenarios:** RC-02, RC-03, RC-04.
...
5. F-05 — define the host compatibility matrix and policy.
6. F-06 — centralize session-scoped cleanup.
7. F-07 — remove mutable session-id state from the adapter.
8. F-08 to F-13 — close the executable coverage gaps.
