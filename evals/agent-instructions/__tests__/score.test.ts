...
describe('scoreCase', () => {
  it('returns PASS when all hard assertions pass', () => {
    const result = scoreCase('test', 'repository_contributor', [
  ...
  it('returns FAIL when any hard assertion fails', () => {
    const result = scoreCase('test', 'flowguard_product', [
...
describe('summarizeResults', () => {
  it('counts PASS, FAIL, and RUNNER_ERROR separately', () => {
    const results = [
      scoreCase('a', 'repository_contributor', [{ description: 'x', type: 'exit_code', severity: 'hard', passed: true }], 10),
      scoreCase('b', 'repository_contributor', [{ description: 'x', type: 'exit_code', severity: 'hard', passed: false }], 10),
      scoreCase('c', 'flowguard_product', [], 10, 'timeout'),
      scoreCase('d', 'flowguard_product', [{ description: 'x', type: 'exit_code', severity: 'hard', passed: true }], 10),
    ];
  ...
  it('sorts cases by insertion order', () => {
    const results = [
      scoreCase('b', 'repository_contributor', [{ description: 'x', type: 'exit_code', severity: 'hard', passed: true }], 10),
      scoreCase('a', 'flowguard_product', [{ description: 'x', type: 'exit_code', severity: 'hard', passed: true }], 10),
    ];
    const summary = summarizeResults('fake', results);
