...
| R3   | HIGH        | Fixed   | Fixed via #585. Central sink-layer redaction applies to all log extras, so unknown identity metadata in logs is sanitized by default; see Logging Redaction below. |
...
| R5   | MEDIUM      | Fixed           | Fixed via #585. Windows + UNC path redaction implemented in the central log redactor; see Logging Redaction below.                                            |
| R6   | MEDIUM      | Fixed           | Fixed via #585. Central `redactExtra` deep-walks nested objects/arrays in the logging pipeline.                                                               |
| R7   | MEDIUM      | Fixed           | Fixed via #585. Central redactors are null/undefined-safe; covered by tests.                                                                                  |
| R8   | MEDIUM      | Fixed           | Fixed via #585. Conservative secret-value scanning (bearer/JWT/sk-/key=value) added to the central log redactor. Export/archive pipelines tracked separately. |
...
| D1   | LOW        | Fixed        | Fixed via #585. Central sink-layer redaction sanitizes diagnostic strings regardless of call site; see Logging Redaction below.                  |
