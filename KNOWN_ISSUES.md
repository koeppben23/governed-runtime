...
| R3   | HIGH        | Fixed (logging) | Central sink-layer redaction applies to all log extras, so unknown identity metadata in logs is sanitized by default; see Logging Redaction below. |
| R4   | HIGH        | Open            | Telemetry error/status export needs scrubbing.                                                                                                     |
| AUD2 | HIGH        | Open            | Audit write lock needs stale-lock recovery.                                                                                                        |
| MUT1 | MEDIUM-HIGH | Tracked         | Mutation scope lost review-gate + multi-mode/canonical modules after the orchestrator split (see Mutation Coverage below).                         |
...
| C14  | MEDIUM      | Fixed           | Repo-local tarball install flow must generate checksum evidence required by default-on verification. #494                                     |
...
| R5   | MEDIUM      | Fixed (logging) | Windows + UNC path redaction implemented in the central log redactor; see Logging Redaction below.                                            |
| R6   | MEDIUM      | Fixed (logging) | Central `redactExtra` deep-walks nested objects/arrays in the logging pipeline.                                                               |
| R7   | MEDIUM      | Fixed (logging) | Central redactors are null/undefined-safe; covered by tests.                                                                                  |
| R8   | MEDIUM      | Fixed (logging) | Conservative secret-value scanning (bearer/JWT/sk-/key=value) added to the central log redactor. Export/archive pipelines tracked separately. |
...
| D1   | LOW        | Fixed (logging) | Central sink-layer redaction sanitizes diagnostic strings regardless of call site; see Logging Redaction below.                                  |
| SZ1  | LOW-MEDIUM | Open            | Prod files near the 750-LOC blocker: `evidence-artifacts.ts` (684), `policy-snapshot-normalize.ts` (677). `plugin-audit.ts` relieved 706 -> 590. |
| SZ2  | LOW        | Open            | `dependency-rules.test.ts` (1501 LOC) is near the 1500 advisory; split without breaking the cycle-detection logic when it next grows.            |
...
Resolved on branch `chore/mutation-scope-and-size-hardening` (pending merge):
added unit tests for the two previously untested modules and added all five
files to `stryker.conf.json` + `vitest.stryker.config.ts`. Verified per-file
mutation scores (break threshold 80): `agent-resolution.ts` 100,
`orchestrator-output.ts` 100, `canonical-json.ts` 100,
`review-validation-mode.ts` 90.2, `orchestrator-detection.ts` 90.4. MUT1 stays
`Tracked` until the branch merges.
...
tracked separately. (Pending merge of `fix/logging-hardening-578-584`.)
