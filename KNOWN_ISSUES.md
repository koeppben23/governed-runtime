...
| CE2 | LOW-MEDIUM | Fixed | Canonical linkage is a COHERENT relation, not identifier equality: `obligation.invocationId === invocation.invocationId` AND the invocation must back-reference the same `obligationId` AND `obligationType`. Obligations without linkage resolve nothing; the newest-findingsHash obligationId fallback is removed and incoherent back-references are rejected at the schema boundary (`refineAssuranceInvocationLinkageCoherence`). |
