...
| SZ1  | LOW-MEDIUM | Fixed        | Prod files near the 650-LOC blocker: none. `src/adapters/workspace/evidence-artifacts.ts` is 209 LOC and `src/integration/plugin-audit.ts` is 461 LOC (both well below the limit); `src/config/policy-snapshot-normalize.ts` was removed. |
