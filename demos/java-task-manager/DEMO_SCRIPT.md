...
| Explain the exhausted gate (if it occurs) | ARCH_REVIEW | "Ist das Review-Budget erschopft und wurden Anderungen verlangt, wird dieses Gate zum Governance-Override-Gate: `/approve` wird mit `GOVERNANCE_OVERRIDE_REQUIRED` geblockt. Erlaubt sind nur `/override-approve`, `/request-changes` und `/reject`; der Override bindet exakt den gepruften ADR-Digest." |
...
| `/override-approve` (optional, only if legitimate) | "Der Override wird explizit auditiert und bindet exakt den gepruften Digest. Das ist die belastbare Unternehmensaussage: Ein Mensch hat die Verantwortung fur eine nicht konvergierte Review bewusst ubernommen — kein stiller Fail-Open." |
...
| Explain the exhausted gate | EVIDENCE_REVIEW                | "Ist das Implementierungs-Review erschöpft und wurden Änderungen verlangt, wird das Gate zum Governance-Override-Gate: `/approve` wird mit `GOVERNANCE_OVERRIDE_REQUIRED` geblockt. Erlaubt sind nur `/override-approve`, `/request-changes` und `/reject`; der Override bindet exakt den geprüften Implementation-Digest." |
...
| Point out `actionGuidance` | "`create PR` und `export evidence` sind hier `recommended`. Das sind reine Präsentations-Hinweise — keine Freigabe. Die eigentliche Fail-Closed-Durchsetzung bleibt beim `/export`-Rail und den Gates. `abandon` erscheint als Exit-Option, nie als verboten."                      |
