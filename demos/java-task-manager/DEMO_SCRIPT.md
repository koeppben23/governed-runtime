...
### Step 2b — Optional: Prove Host-Tool Enforcement (the forbidden mutation)

Optional — run this only when time allows; it is the same enforcement boundary
one level lower. The mutation attempt is model-dependent: if the agent refuses
on its own, the denial story still holds and the recording in `FALLBACK.md`
covers it.

| Action                                                            | Phase  | What I Say                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ----------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Ask the agent to modify a Java file directly (e.g. `TaskService`) | TICKET | "Ich bitte den Agenten, direkt eine Datei zu ändern — ohne Plan, ohne Freigabe. Mutierende Host-Tools (`edit`, `write`, `bash`, `apply_patch`) sind nur in IMPLEMENTATION erlaubt: der Plugin-Hook blockt den Aufruf synchron mit `HOST_TOOL_PHASE_DENIED`, und die Runtime schreibt die Ablehnung als `enforcement:denied`-Event in den Audit-Trail. Der Agent kann die Phase nicht umgehen — er kann nur den erlaubten Weg nehmen." |
| Point out the audit evidence                                      | —      | "`enforcement:denied` ist kein Log-Eintrag, sondern ein Audit-Event im kanonischen Trail der Session. Nach `/export` liegt es im Export-Paket (`audit/audit.jsonl`) und ist mit `verify-evidence-package.mjs` offline prüfbar."                                                                                                                                                                                                       |
...
| Verify the export package (optional) | "Das `/export`-Paket kann ein Empfänger ohne diese Session offline nachrechnen: `verify-evidence-package.mjs` prüft Checksumme, Dateiinventur, kanonischen Content-Digest, Session-Zuordnung und die archivierte Audit-Kette und schlägt bei jeder Manipulation fehl. Umfang und Grenzen — Offline-TSA, externe Publication-Bindung, fehlende Signatur — stehen in `EVIDENCE_PACKAGE.md`."                                                                                                                                                      |
