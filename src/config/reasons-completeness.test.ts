...
describe('P10c — reason code split', () => {
  it('all 150 codes from split arrays are registered exactly once (no duplicates)', async () => {
    ...
    expect(allSplitCodes).toHaveLength(150);
    // No duplicates across the 3 arrays
    expect(new Set(allSplitCodes).size).toBe(150);
  ...
  it('VALIDATION_REASONS has exactly 66 entries', async () => {
    const { VALIDATION_REASONS } = await import('./reasons-validation.js');
    expect(VALIDATION_REASONS.length).toBe(66);
