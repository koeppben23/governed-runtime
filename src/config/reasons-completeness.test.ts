...
describe('P10c — reason code split', () => {
  it('all 192 codes from split arrays are registered exactly once (no duplicates)', async () => {
    ...
    expect(allSplitCodes).toHaveLength(192);
    // No duplicates across the 3 arrays
    expect(new Set(allSplitCodes).size).toBe(192);
