...
describe('P10c — reason code split', () => {
  it('all 137 codes from split arrays are registered exactly once (no duplicates)', async () => {
    ...
    expect(allSplitCodes).toHaveLength(137);
    // No duplicates across the 3 arrays
    expect(new Set(allSplitCodes).size).toBe(137);
  ...
  it('INFRA_REASONS has exactly 35 entries', async () => {
    const { INFRA_REASONS } = await import('./reasons-infra.js');
    expect(INFRA_REASONS.length).toBe(35);
