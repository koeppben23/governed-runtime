...
describe('P10c — reason code split', () => {
  it('all 234 codes from split arrays are registered exactly once (no duplicates)', async () => {
    ...
    expect(allSplitCodes).toHaveLength(234);
    // No duplicates across the 4 arrays
    expect(new Set(allSplitCodes).size).toBe(234);
  ...
  it('VALIDATION_REASONS has exactly 85 entries', async () => {
    const { VALIDATION_REASONS } = await import('./reasons-validation.js');
    expect(VALIDATION_REASONS.length).toBe(85);
