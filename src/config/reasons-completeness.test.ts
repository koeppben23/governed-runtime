...
describe('P10c — reason code split', () => {
  it('all 279 codes from split arrays are registered exactly once (no duplicates)', async () => {
    ...
    expect(allSplitCodes).toHaveLength(279);
    // No duplicates across the 5 arrays
    expect(new Set(allSplitCodes).size).toBe(279);
  ...
  it('MUTATION_REASONS has exactly 7 entries', async () => {
    const { MUTATION_REASONS } = await import('./reasons-mutation.js');
    expect(MUTATION_REASONS.length).toBe(7);
