...
describe('P10c — reason code split', () => {
  it('all 273 codes from split arrays are registered exactly once (no duplicates)', async () => {
    ...
    expect(allSplitCodes).toHaveLength(273);
    // No duplicates across the 5 arrays
    expect(new Set(allSplitCodes).size).toBe(273);
