...
describe('P10c — reason code split', () => {
  it('all 278 codes from split arrays are registered exactly once (no duplicates)', async () => {
    ...
    expect(allSplitCodes).toHaveLength(278);
    // No duplicates across the six arrays.
    expect(new Set(allSplitCodes).size).toBe(278);
