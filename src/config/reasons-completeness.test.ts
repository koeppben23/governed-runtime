...
describe('P10c — reason code split', () => {
  it('all 149 codes from split arrays are registered exactly once (no duplicates)', async () => {
    ...
    expect(allSplitCodes).toHaveLength(149);
    // No duplicates across the 3 arrays
    expect(new Set(allSplitCodes).size).toBe(149);
