...
describe('P10c — reason code split', () => {
  it('all 118 codes from split arrays are registered exactly once (no duplicates)', async () => {
    ...
    expect(allSplitCodes).toHaveLength(118);
    // No duplicates across the 3 arrays
    expect(new Set(allSplitCodes).size).toBe(118);
  ...
  it('VALIDATION_REASONS has exactly 49 entries', async () => {
    const { VALIDATION_REASONS } = await import('./reasons-validation.js');
    expect(VALIDATION_REASONS.length).toBe(49);
  ...
  it('INFRA_REASONS has exactly 28 entries', async () => {
    const { INFRA_REASONS } = await import('./reasons-infra.js');
    expect(INFRA_REASONS.length).toBe(29);
