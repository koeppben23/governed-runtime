...
describe('persistence', () => {
  ...
  describe('HAPPY', () => {
    ...
    it('migrates legacy claim assertionId to assertion identity', async () => {
      const state = makeProgressedState('PLAN_REVIEW');
      const json = JSON.parse(JSON.stringify(state)) as Record<string, unknown>;
      const plan = json.plan as Record<string, unknown>;
      plan.claimDeclarations = {
        ...
        claims: [
          {
            claimId: '00000000-0000-4000-8000-000000000001',
            statement: 'claim with legacy assertion',
            critical: true,
            authoritySectionId: 's1',
            expectedCheckId: 'test',
            counterexampleRequirement: {
      ...
      const loaded = await readState(tmpDir);
      const claim = loaded?.plan?.claimDeclarations?.claims[0];
      const req = claim?.counterexampleRequirement as Record<string, unknown>;
      expect(req).toBeDefined();
      expect(req.mode).toBe('assertion');
      expect(req.assertion).toEqual({ providerId: 'junit', localId: 'com.example.Test#method' });
      expect(req.assertionId).toBeUndefined();
    });

    it('migrates legacy evidence assertionId and framework', async () => {
      ...
      const loaded = await readState(tmpDir);
      const attempt = loaded?.validationAttempts[0] as Record<string, unknown> | undefined;
      const extraction = (attempt?.result as Record<string, unknown>)?.assertionExtraction as
        Record<string, unknown> | undefined;
      const assertions = extraction?.assertions as Array<Record<string, unknown>> | undefined;
      expect(assertions).toHaveLength(1);
      expect(assertions![0]!.assertion).toEqual({
        providerId: 'junit',
        localId: 'com.example.Test#method',
      });
      expect(assertions![0]!.providerId).toBe('junit');
      expect(assertions![0]!.assertionId).toBeUndefined();
      expect(assertions![0]!.framework).toBeUndefined();
    });

    it('migrates go: prefix alias to go_test providerId', async () => {
      const state = makeProgressedState('PLAN_REVIEW');
      const json = JSON.parse(JSON.stringify(state)) as Record<string, unknown>;
      const plan = json.plan as Record<string, unknown>;
      plan.claimDeclarations = {
        ...
        claims: [
          {
            ...
            counterexampleRequirement: {
              mode: 'assertion',
              checkId: 'security',
              assertionId: 'go:example.com/pkg::TestName',
            },
          },
        ],
      };
      ...
      const loaded = await readState(tmpDir);
      const req = loaded?.plan?.claimDeclarations?.claims[0]?.counterexampleRequirement as Record<
        string,
        unknown
      >;
      expect(req.assertion).toEqual({
        providerId: 'go_test',
        localId: 'example.com/pkg::TestName',
      });
    });

    it('invalidates approvalCertificate when legacy assertion claims are migrated', async () => {
      const state = makeProgressedState('PLAN_REVIEW');
      const json = JSON.parse(JSON.stringify(state)) as Record<string, unknown>;
      const plan = json.plan as Record<string, unknown>;
      plan.claimDeclarations = {
    ...
    it('rejects legacy claim assertionId with unknown prefix', async () => {
      const state = makeProgressedState('PLAN_REVIEW');
      const json = JSON.parse(JSON.stringify(state)) as Record<string, unknown>;
      const plan = json.plan as Record<string, unknown>;
      plan.claimDeclarations = {
      ...
      try {
        ...
        expect((err as PersistenceError).code).toBe('SCHEMA_VALIDATION_FAILED');
        expect((err as PersistenceError).message).toContain('Unknown legacy assertion prefix');
      }
    });

    it('rejects legacy evidence with prefix/framework mismatch', async () => {
      const state = makeProgressedState('IMPL_REVIEW');
      const json = JSON.parse(JSON.stringify(state)) as Record<string, unknown>;
      json.implementation = {
        changedFiles: ['test.ts'],
      ...
      json.validationAttempts = [
        {
          attemptId: '00000000-0000-4000-8000-0000000000b1',
          scope: 'implementation',
          implementationDigest: 'impl-digest',
          result: {
            checkId: 'security',
      ...
      try {
        ...
        expect((err as PersistenceError).code).toBe('SCHEMA_VALIDATION_FAILED');
        expect((err as PersistenceError).message).toContain('does not match framework');
      }
    });

    it('rejects legacy evidence with non-string assertionId via frozen schema', async () => {
      const state = makeProgressedState('IMPL_REVIEW');
      const json = JSON.parse(JSON.stringify(state)) as Record<string, unknown>;
      json.implementation = {
        changedFiles: ['test.ts'],
      ...
      json.validationAttempts = [
        {
          ...
          result: {
            ...
            assertionExtraction: {
              status: 'extracted',
              attemptId: '00000000-0000-4000-8000-0000000000c2',
              format: 'junit_xml',
              reportDigests: ['b'.repeat(64)],
              assertions: [
                {
