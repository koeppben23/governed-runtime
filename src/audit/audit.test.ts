...
function buildSessionTrail(): AuditEvent[] {
  return [
    ...
    makeAuditEvent({
      ...
      phase: "PLAN",
      timestamp: TS1,
      detail: { kind: "transition", from: "TICKET", to: "PLAN", event: "TICKET_SET", autoAdvanced: false, chainIndex: -1 },
    }),
    makeAuditEvent({
      event: "transition:PLAN_READY",
      phase: "PLAN",
      timestamp: TS2,
      detail: { kind: "transition", from: "PLAN", to: "PLAN_REVIEW", event: "PLAN_READY", autoAdvanced: true, chainIndex: 0 },
    }),
    makeAuditEvent({
    ...
    makeAuditEvent({
      ...
      timestamp: TS2,
      detail: { kind: "transition", from: "VALIDATION", to: "IMPLEMENTATION", event: "ALL_PASSED", autoAdvanced: false, chainIndex: -1 },
    }),
    makeAuditEvent({
      event: "transition:IMPL_COMPLETE",
      phase: "IMPL_REVIEW",
      timestamp: TS3,
      detail: { kind: "transition", from: "IMPLEMENTATION", to: "IMPL_REVIEW", event: "IMPL_COMPLETE", autoAdvanced: false, chainIndex: -1 },
    }),
    makeAuditEvent({
      event: "transition:REVIEW_CONVERGED",
      phase: "EVIDENCE_REVIEW",
      timestamp: TS3,
      detail: { kind: "transition", from: "IMPL_REVIEW", to: "EVIDENCE_REVIEW", event: "REVIEW_CONVERGED", autoAdvanced: false, chainIndex: -1 },
    }),
    makeAuditEvent({
      event: "transition:APPROVE",
      phase: "COMPLETE",
      timestamp: TS3,
      detail: { kind: "transition", from: "EVIDENCE_REVIEW", to: "COMPLETE", event: "APPROVE", autoAdvanced: false, chainIndex: -1 },
    }),
    makeAuditEvent({
...
describe("audit types", () => {
  ...
  describe("EDGE", () => {
    ...
    it("factory event names encode the kind as prefix", () => {
      const t = createTransitionEvent(SESSION_ID, "PLAN", { from: "TICKET", to: "PLAN", event: "PLAN_READY", autoAdvanced: false, chainIndex: -1 }, TS1, GENESIS_HASH);
  ...
  describe("EDGE", () => {
    ...
    it("insertion attack detected — new event breaks prevHash chain", () => {
      ...
      const inserted = createTransitionEvent(
        SESSION_ID, "PLAN",
        { from: "PLAN", to: "PLAN_REVIEW", event: "PLAN_READY", autoAdvanced: false, chainIndex: -1 },
        TS2, chain[0]!.chainHash, // Uses correct prevHash for [0]
      );
    ...
    it("getLastChainHash skips trailing legacy events", () => {
      ...
      const mixed = [
        ...chain.map(e => e as unknown as Record<string, unknown>),
        legacy,
      ];
...
describe("audit query", () => {
  ...
  describe("EDGE", () => {
    ...
    it("complex composed query", () => {
      ...
      const filter = allOf(
        bySession("sess-a"),
        anyOf(byKind("transition"), byKind("error")),
      );
...
describe("audit summary", () => {
  ...
  describe("HAPPY", () => {
    it("generateTimeline produces correct timeline for a complete session", () => {
      ...
      expect(timeline.phaseProgression).toEqual([
        "TICKET", "PLAN", "VALIDATION", "IMPLEMENTATION",
        "IMPL_REVIEW", "EVIDENCE_REVIEW", "COMPLETE",
      ]);
    });
  ...
  describe("CORNER", () => {
    ...
    it("compliance with error events that are resolved (session reaches COMPLETE)", () => {
      const trail: AuditEvent[] = [
        ...
        makeAuditEvent({ event: "error:TOOL_TIMEOUT", phase: "PLAN", timestamp: TS2, actor: "machine", detail: { kind: "error", code: "TOOL_TIMEOUT" } }),
  ...
  describe("EDGE", () => {
    ...
    it("compliance: review gates not applicable if session never reached those phases", () => {
      const trail: AuditEvent[] = [
        makeAuditEvent({ event: "lifecycle:session_created", phase: "TICKET", timestamp: TS1, actor: "system", detail: { kind: "lifecycle", action: "session_created", finalPhase: "TICKET" } }),
        makeAuditEvent({ event: "lifecycle:session_aborted", phase: "TICKET", timestamp: TS2, actor: "user", detail: { kind: "lifecycle", action: "session_aborted", finalPhase: "TICKET" } }),
      ];
      const summary = generateComplianceSummary(trail, SESSION_ID, null, TS3);
...
describe("audit completeness", () => {
  ...
  describe("CORNER", () => {
    ...
    it("evidenceReviewDecision slot at COMPLETE with error → missing", () => {
      const state = makeState("COMPLETE", {
        ...makeProgressedState("COMPLETE"),
        error: { code: "FATAL", message: "Something broke", recoveryHint: "restart", occurredAt: FIXED_TIME },
      });
      const report = evaluateCompleteness(state);
