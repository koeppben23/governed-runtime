/**
 * @module discovery/scoped-stack-extractors
 * @description Manifest scope and nested stack fact extraction for scoped stacks.
 *
 * Recognizes module roots from manifest paths and extracts stack facts from
 * nested manifest contents (package.json, pom.xml, Cargo.toml, pyproject.toml,
 * docker-compose). The projection that binds facts to scopes lives in
 * `scoped-stack.ts`.
 */
 ...
 */
export function extractScopePath(filePath: string): string | null {
...
export type ScopeFact = { id: string; kind: DetectedStackTarget; version?: string };

/** A stack fact bound to the manifest file it was extracted from. */
export type ScopedFactWithEvidence = ScopeFact & { evidence: string };
...
export async function readNestedManifestFacts(
