...
const HEADINGS = ['### For Plans', '### For Implementations', '### For Architecture Decisions (ADRs)', '### Content Review (for /review flow)'] as const;
...
describe('renderReviewerTaskPrompt phase criteria', () => {
  it('selects plan criteria from the frozen artifact contract when legacy callers omit reviewType', () => {
    const prompt = renderReviewerTaskPrompt({
      ...
      challengeContract: { requiredChallengeCount: 1, requiredChallengeKind: 'design_challenge', evidenceRefs: [{ kind: 'plan_adr_section' }] },
    });
  ...
  it('selects implementation criteria from the host-enforced implementation contract', () => {
    const prompt = renderReviewerTaskPrompt({
      ...
      challengeContract: { requiredChallengeCount: 1, requiredChallengeKind: 'implementation_challenge', evidenceRefs: [{ kind: 'implementation' }] },
    });
  ...
  it('selects content criteria from the content challenge contract', () => {
    const prompt = renderReviewerTaskPrompt({
      ...BASE,
      challengeContract: { requiredChallengeCount: 1, requiredChallengeKind: 'content_challenge', evidenceRefs: [{ kind: 'content' }] },
    });
