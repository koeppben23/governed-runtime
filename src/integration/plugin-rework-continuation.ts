 ...
 *
 * The persisted `implementationRework` marker is cleared by every re-record
 * (IMPLEMENTATION → IMPL_VALIDATION → back on a failing fresh check), so the
 * continuation cannot key on that state. The latch flips on when a /check
...
export async function isAllowedReworkContinuation(
  ...
  // The scope-local latch survives the re-record that clears the persisted
  // marker, so a failing FRESH check after re-record still permits the next
  // repair. The marker remains the entry evidence; an exhausted budget never
  // unlocks regardless of the latch.
