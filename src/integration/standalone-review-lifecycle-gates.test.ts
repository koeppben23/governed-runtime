 ...
 * @description Contract gates for the peer review claim lifecycle:
...
describe('peer review lifecycle gates', () => {
  it('gate 4: output repair keeps the logical task — same 3 projected claimIds', () => {
    const args = { branch: 'feature/x', base: 'main' };
    const prepared = preparePeerReviewEvidence(args, NOW, undefined, TASK_A, OBLIGATION_A);
    ...
    const evidence = appendCompletedReviewEvidence({
      ...
      prepared: preparePeerReviewEvidence(args, NOW, undefined, TASK_A, OBLIGATION_A),
      completedAt: NOW,
    });

    const projection = deriveProofGraph(
      makeState('PEER_REVIEW_COMPLETE', {
        peerReviewEvidence: evidence,
  ...
  it('gate 5: a genuinely new /review creates a new logical task with distinct claims', () => {
    ...
    const preparedA = preparePeerReviewEvidence(argsA, NOW, undefined, TASK_A, OBLIGATION_A);
    const preparedB = preparePeerReviewEvidence(argsB, NOW, undefined, TASK_B, OBLIGATION_B);
    const evidenceA = appendCompletedReviewEvidence({
      evidence: appendPreparedReviewEvidence([], preparedA),
      prepared: preparePeerReviewEvidence(argsA, NOW, undefined, TASK_A, OBLIGATION_A),
      completedAt: NOW,
    });
    const evidenceB = appendCompletedReviewEvidence({
      evidence: appendPreparedReviewEvidence([], preparedB),
      prepared: preparePeerReviewEvidence(argsB, NOW, undefined, TASK_B, OBLIGATION_B),
      completedAt: NOW,
    });
