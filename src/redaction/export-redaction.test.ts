...
describe("redaction/export-redaction", () => {
  ...
  describe("HAPPY", () => {
    ...
    it("redacts fourEyes initiatedBy and decidedBy in review report", () => {
      ...
      const fe = (output.completeness as Record<string, unknown>).fourEyes as Record<string, unknown>;
    ...
    it("redacts completeness slots detail in review report", () => {
      ...
      const output = redactReviewReport(input, "basic") as Record<string, unknown>;
      const slots = ((output.completeness as Record<string, unknown>).slots as Array<Record<string, unknown>>);
  ...
  describe("EDGE", () => {
    it("strict mode token is consistent across multiple calls with same input", () => {
      ...
      const results = Array.from({ length: 5 }, () =>
        redactDecisionReceipts(input, "strict"),
      );
      const tokens = results.map((r) => String((r.receipts[0] as Record<string, unknown>).decidedBy));
      expect(new Set(tokens).size).toBe(1);
    });

    it("strict mode produces different tokens for different field values", () => {
      const out = redactDecisionReceipts({ receipts: [{ decidedBy: "alice", rationale: "bob" }] }, "strict");
      const r = out.receipts[0] as Record<string, unknown>;
    ...
    it("strict mode review report findings message", () => {
      ...
      expect(String((out.findings[0] as Record<string, unknown>).message)).toMatch(/^\[REDACTED:[a-f0-9]{12}\]$/);
    });
