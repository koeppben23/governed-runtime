...
describe("review rail", () => {
  ...
  describe("BAD", () => {
    it("flags missing ticket as warning", async () => {
      ...
      const report = await executeReview(state, NOW);
      const ticketFinding = report.findings.find(f =>
        f.category === "completeness" && f.message.includes("ticket"),
      );
    ...
    it("flags missing plan as warning", async () => {
      ...
      const report = await executeReview(state, NOW);
      const planFinding = report.findings.find(f =>
        f.category === "completeness" && f.message.includes("plan"),
      );
