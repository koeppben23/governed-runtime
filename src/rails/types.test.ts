...
import { applyTransition, autoAdvance, runConvergenceLoop, runSingleIteration, createPolicyEvalFn, DEFAULT_MAX_REVIEW_ITERATIONS } from "../rails/types";
import type { RailContext, ConvergenceResult, IterationResult } from "../rails/types";
import { evaluate } from "../machine/evaluate";
import { makeState, makeProgressedState, TICKET, PLAN_RECORD, SELF_REVIEW_CONVERGED, VALIDATION_PASSED, IMPL_EVIDENCE, IMPL_REVIEW_CONVERGED } from "../__fixtures__";
...
describe("rails/types", () => {
  ...
  describe("HAPPY", () => {
    it("applyTransition returns new state with updated phase", () => {
      ...
      const next = applyTransition(state, "TICKET", "PLAN", "PLAN_READY", "2026-01-01T00:00:00.000Z");
    ...
    it("applyTransition clears error", () => {
      const state = makeState("TICKET", { error: { code: "ERR", message: "err", recoveryHint: "h", occurredAt: "2026-01-01T00:00:00.000Z" } });
      const next = applyTransition(state, "TICKET", "PLAN", "PLAN_READY", "2026-01-01T00:00:00.000Z");
      expect(next.error).toBeNull();
    });
    ...
    it("createPolicyEvalFn returns a function that evaluates with policy", () => {
      const policyCtx: RailContext = { ...ctx, policy: { mode: "solo", requireHumanGates: false, maxSelfReviewIterations: 1, maxImplReviewIterations: 1, allowSelfApproval: true, audit: { emitTransitions: true, emitToolCalls: true, enableChainHash: false }, actorClassification: {} } };
