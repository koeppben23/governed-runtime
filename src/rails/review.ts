 ...
 *
 * 2. Flow start (startReviewFlow): Peer review entry.
 *    READY → PEER_REVIEW via state machine transition, without advancing
 *    further. The tool layer generates and persists the report, then calls
 *    autoAdvance so the reviewDone guard can fire with report evidence.
 *
 * @version v2
 */
...
import { applyTransition, buildFlowSelectionTransition } from './types.js';
...
/**
 * Transition from READY to PEER_REVIEW phase.
 * Returns the PEER_REVIEW state WITHOUT auto-advancing to PEER_REVIEW_COMPLETE.
...
export function startReviewFlow(state: SessionState, ctx: RailContext): RailResult {
  ...
  return {
    kind: 'ok',
    state: reviewState,
    evalResult: { kind: 'pending', phase: 'PEER_REVIEW' },
