...
4. Derive structured claim declarations for the plan: each names a falsifiable statement (\`statement\`), its governing section (\`authoritySectionId\`), whether it is \`critical\`, a fresh \`claimId\` UUID, and the \`expectedCheckId\` that must pass after implementation. Critical claims also require a distinct \`counterexampleCheckId\`: one execution cannot serve as both confirmation and adversarial falsification.
