...
4. Derive structured claim declarations for the plan: each names a falsifiable statement (\`statement\`), its governing section (\`authoritySectionId\`), whether it is \`critical\`, a fresh \`claimId\` UUID, and the \`expectedCheckId\` that must pass after implementation. Critical claims also require a \`counterexampleRequirement\`: \`{ checkId: "...", assertion: { providerId: "...", localId: "..." } }\` where \`assertion.providerId\` identifies the test framework (e.g., \`junit\`) and \`assertion.localId\` identifies the specific test assertion whose failure contradicts the claim (e.g., \`com.example.Test#method\`).
