...
4. For a response that transitions to a stop phase, inspect that \`flowguard_run_check\` response before calling any other tool:
   - If its phase is \`IMPLEMENTATION\` while you are INSIDE this /check's repair-recheck continuation (a \`changes_requested\` verdict was already recorded earlier in this /check invocation — this stays true even across a re-record, which clears the persisted \`implementationRework\` marker): do NOT stop — continue the automatic repair-recheck cycle: repair the code, call \`flowguard_implement({})\` again to re-record, re-run the checks, resolve open implementation challenges, and re-review.
