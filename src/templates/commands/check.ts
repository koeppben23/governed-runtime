...
4. For a response that transitions to a stop phase, inspect that \`flowguard_run_check\` response before calling any other tool:
   - If its phase is \`IMPLEMENTATION\` and the session carries an ACTIVE non-exhausted \`implementationRework\` marker (a prior \`changes_requested\` verdict was already recorded during this /check): do NOT stop — continue the automatic repair-recheck cycle described in the \`changes_requested\` branch below: repair the code, re-record, re-run the checks, resolve open implementation challenges, and re-review.
   - If its phase is \`IMPLEMENTATION\` without an active rework marker: report the passed checks and the rendered next action, then STOP. Do not call \`read\`, \`glob\`, \`grep\`, \`bash\`, \`write\`, \`edit\`, \`flowguard_implement\`, or any implementation-review tool. Only a new, explicit user \`/implement\` command may start implementation.
...
- When checks advance to \`IMPLEMENTATION\`: a fresh phase without an active rework marker was reported and the command stopped without implementation work; an active rework marker was continued automatically through repair-recheck until the review loop's terminal response.
