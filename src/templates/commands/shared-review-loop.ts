...
export function SHARED_REVIEW_LOOP(p: ReviewLoopParams): string {
  ...
  return `   - \`reviewVerdict\` records the INDEPENDENT REVIEWER's result, never your own approval. On convergence it only advances to the human review gate, where the USER approves the ${p.artifactName} via /review-decision. \`flowguard_decision\` is the only user approval.
   ...
   - When \`next\` starts with "INDEPENDENT_REVIEW_REQUIRED":
       1. Call the ${REVIEWER_SUBAGENT_TYPE} subagent via Task tool${p.subagentExtra}. Issue this Task call SEQUENTIALLY — only AFTER the preceding \`${verdictTool}\` call has returned and you have read its \`reviewObligation\`/\`iteration\` from the response; never fire the reviewer Task in parallel with a \`${verdictTool}({ reviewVerdict })\` submission. A parallel reviewer Task binds against a stale or not-yet-created obligation and is correctly blocked fail-closed as \`HOST_SUBAGENT_TASK_REQUIRED\` with \`no_matched_record\` (an expected first-attempt outcome, not a defect); recover by re-invoking the reviewer sequentially for the current obligation. If the tool response includes a \`reviewerTaskPrompt\` field, pass it VERBATIM as the Task tool "prompt" argument (append the ${p.artifactName} content and the compact Discovery context below it) — this canonical prompt already carries the required review context (iteration/planVersion) and attestation, so the first Task attempt is not blocked with \`SUBAGENT_PROMPT_MISSING_CONTEXT\`. Only free-compose a prompt if no \`reviewerTaskPrompt\` is provided. Pass the compact Discovery context captured in Phase 1 (health, drift, detectedStack, verificationCandidates, risk surfaces), and instruct the subagent to check Discovery health and drift BEFORE any repo-dependent quality claim and to mark Discovery-dependent claims NOT_VERIFIED when they cannot be correlated to local repository Discovery.
