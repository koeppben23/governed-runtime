 ...
 * FLOWGUARD_MANDATES_KERNEL and derives from the full semantic projection. The repository root AGENTS.md is local contributor
...
const MANDATES_BASE = FLOWGUARD_MANDATES_FULL_BODY.replace(VERSION_FOOTER, '');
...
const REVIEWER_TASK_SECTIONS: readonly string[] = [
  '## Your Role',
  '## Review Approach',
  '## Review Criteria',
...
describe('mandate authority guard', () => {
  it('keeps the installed governance kernel canonical', () => {
  ...
  it('keeps all full-projection mandate sections ordered', () => {
  ...
  it('keeps reviewer task contracts out of root AGENTS.md and installed mandates', () => {
    for (const section of REVIEWER_TASK_SECTIONS) {
      expect(AGENTS_MD.includes(section), `AGENTS.md must not contain "${section}"`).toBe(false);
      expect(
        FLOWGUARD_MANDATES_KERNEL.includes(section),
        `FLOWGUARD_MANDATES_KERNEL must not contain "${section}"`,
      ).toBe(false);
    }
